...
export default function BlockCanvas({ project }: BlockCanvasProps) {
  ...
  useEffect(() => {
    if (project && blocks.length > 0) {
      console.log('Blocks state in useEffect:', blocks); // Added for debugging
      const pythonCode = generatePythonCode(blocks);
      console.log('Generated Python Code:', pythonCode); // Added for debugging
      updateProject({
        id: project.id,
        updates: {
          blocks: blocks,
  ...
  }, []); // Corrected closing for handleDrop useCallback
